...
jest.mock("../../src/services/getOrderResult", () => {
    return {
        ...
        default: jest.fn().mockImplementation(
            async ({ orderId }: GetOrderResultParams) => orderResults[orderId]
        ),
    };
});
...
const encryptedTextMessage = {
    ...
    ephemPublicKey: 'BPEKymabP0ddxQc20+r2WxH4cL5G+HTJgd0uZcpg1tTjunfxJ3miZY1cwpv4AFrLT5u1KM2rLdYzWxzwxUCm7js=',
...
describe('ordersDownloadResult', () => {
    ...
    it('should save order result to txt file when it does not have resource', async () => {
        ...
        expect(await fsPromises.readFile(path.join(process.cwd(), localPath))).toBe(decryptedFileContent)
    });
    ...
    it('should save order result to txt file when it does not have resource', async () => {
        ...
        expect(await fsPromises.readFile(path.join(process.cwd(), localTxtPath))).toBe(decryptedTextMessage)
    });
    ...
    it('should save order encrypted result to txt file when it is not a valid json', async () => {
        ...
        expect(await fsPromises.readFile(path.join(process.cwd(), localTxtPath))).toBe(orderResults['3'].encryptedResult);
    });
});
