    ...
    for (const { actionKey, authorityAccount } of providers) {
      ...
      if (valueOffers.length) {
        const results = await Promise.allSettled(
          valueOffers.map((offer) => new Offer(offer).enable({ from: actionAccount })),
        );
      ...
      if (teeOffers.length) {
        const results = await Promise.allSettled(
          teeOffers.map((offer) => new TeeOffer(offer).enable({ from: actionAccount })),
        );
        results.forEach((res, index) =>
          Printer.print(
            `Tee offer ${teeOffers[index]} was ${res.status === 'fulfilled' ? '' : 'not '}enabled`,
          ),
        );
      }
    }
